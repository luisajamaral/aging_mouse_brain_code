...
snap.pl.umap(data, color="age", interactive=True,  width =450, height = 400,sample_size=100000, marker_opacity=.8)
...
data=snap.read_dataset("h5ads_final/combined_save2.h5ads")
...
get_ipython().run_cell_magic('time', '', 'snap.tl.macs3(data, n_jobs = 20, replicate = \'rep\',groupby=\'celltype_final\',key_added = \'celltype_final_rep\',blacklist="/projects/ps-renlab/yangli/genome/mm10/mm10.blacklist.bed")\ndata.close()\n')


# In[ ]:


snap.tl.macs3(data, n_jobs = 20, replicate = 'rep',
              replicate_qvalue=0.05, qvalue=0.01,groupby='celltype_final',
              key_added = 'celltype_final_rep_01',
...
data=snap.read_dataset("combined.h5ads")
snap.tl.macs3(data, n_jobs = 20, replicate = 'rep',replicate_qvalue=0.05, qvalue=0.01,groupby='celltype_batch_age',key_added = 'celltype_age_batch_01',blacklist="/projects/ps-renlab/yangli/genome/mm10/mm10.blacklist.bed")
data.close()
...
snap.tl.macs3(data, n_jobs = 12, replicate = 'rep',replicate_qvalue=0.05, 
...
data=snap.read_dataset("combined_save2.h5ads")
peak_mat = snap.read("pmat_celltype_final_rep_01.h5ad")
...
data.obs['celltype_region'] = data.obs['celltype_final']+":"+data.obs['region']
data.obs['celltype_region_age'] = data.obs['celltype_final']+":"+data.obs['region']+":"+data.obs['age']
min_pct = 0.01

uniq_cts = data.obs['celltype_region'].unique()
ct = "Oligo NN:HCA"
...
group1 = ct + ":2mo"
group2 = ct + ":18mo"
g1 = data.obs['celltype_region_age'] == group1
g2 = data.obs['celltype_region_age'] == group2
if g1.sum() < 250 or g2.sum() < 250: 
    print("not enough cells\n")
celltype = ct.split(":")[0]

presence_g1 = (peak_mat.X[g1, :] > 0).sum(axis=0) / g1.sum()
presence_g2 = (peak_mat.X[g2, :] > 0).sum(axis=0) / g1.sum()
...
background_peaks_df.to_csv("background_peaks_" + ct.replace(" ", "_") + "_2vs18.csv", index=False)

diff_peaks = snap.tl.diff_test(
    peak_mat,
    cell_group1=g1,
    cell_group2=g2,
    min_log_fc=0.2, min_pct=0,
...
bed_background_peaks.to_csv("background_peaks_" + ct.replace(" ", "_") + "_2vs18.bed", sep="\t", header=False, index=False)
...
down_peaks = significant_peaks[significant_peaks['log2(fold_change)'] > 0.2]
up_peaks = significant_peaks[significant_peaks['log2(fold_change)'] < -0.2]
...
# Save up-regulated and down-regulated peaks to BED files
up_peaks_bed.to_csv("up_" + ct.replace(" ", "_") + "_2vs18.bed", sep="\t", header=False, index=False)
down_peaks_bed.to_csv("down_" + ct.replace(" ", "_") + "_2vs18.bed", sep="\t", header=False, index=False)
...
# Convert peak_mat.var_names to a NumPy array for proper indexing
var_names_array = np.array(peak_mat.var_names)
#var_names_array= var_names_array[peaks_selected]
# Get indices of background peaks
background_indices = np.where(background_mask)[0]
background_peaks = var_names_array[background_indices]
...
presence_g1_series = pd.Series(presence_g1, index=peak_mat.var_names)
presence_g2_series = pd.Series(presence_g2, index=peak_mat.var_names)


# In[ ]:


background_mask = np.ravel((presence_g1 >= 0.025) | (presence_g2 >= 0.025))

# Get indices of background peaks
background_indices = np.where(background_mask)[0]
background_peaks = peak_mat.var_names[background_indices]


# In[ ]:


peak_mat.var_names.typeof()


# In[ ]:


background_peaks = peak_mat.var_names[(presence_g1.flatten() >= 0.025) | (presence_g2.flatten() >= 0.025)]


# In[ ]:


background_peaks = peak_mat.var_names[(presence_g1 >= 0.025) | (presence_g2 >= 0.025)]
...
peaks_selected = peaks[celltype].to_numpy()


# In[ ]:


peak_mat.X[g1, peaks_selected ]
...
presence_g1 = (peak_mat.X[g1, peaks_selected] > 0).sum(axis=0) / g1.sum()

presence_g2 = (peak_mat.X[g2, peaks_selected] > 0).sum(axis=0) / g1.sum()
...
for ct in uniq_cts:
    file = "diff_peaks_" + ct.replace(" ", "_") + "_2vs18_iter.csv"
    ...
    diff_peaks = snap.tl.diff_test(
        ...
        features=peaks_selected#,covariates='batch'
    )
    if diff_peaks.shape[0] == 0 :
        continue
    column_names = diff_peaks.columns
...
peak_mat = snap.pp.make_peak_matrix(data, use_rep=peaks['Peaks'], file = "pmat_celltype_final_rep_01.h5ad")


# In[ ]:


data.obs['celltype_final_reg'] = data.obs['celltype_final']
data.obs['celltype_final_reg'] = data.obs['celltype_final_reg'].apply(lambda x: 'Astro NN' if x in ['Astro-TE NN', 'Astro-NT NN'] else x)


# In[ ]:


data.obs['celltype_region_age'] = data.obs['celltype_final_reg']+":"+data.obs['region']+":"+data.obs['age']
data.obs['celltype_region'] = data.obs['celltype_final_reg']+":"+data.obs['region']
...
for ct in uniq_cts:
    ...
    if g1.sum() < 250 or g2.sum() < 250: 
        ...
        continue
    celltype = ct.split(":")[0]
    if(celltype == "Astro NN"):
        peaks_selected = (peaks['Astro-TE NN'] | peaks['Astro-NT NN']).to_numpy()
    else:
        peaks_selected = peaks[celltype].to_numpy()
    diff_peaks = snap.tl.diff_test(
        peak_mat,
...
data.obs['celltype_batch'] = data.obs['celltype_final']+":"+data.obs['batch']
data.obs['celltype_batch_age'] = data.obs['celltype_final']+":"+data.obs['batch']+":"+data.obs['age']

uniq_cts = data.obs['celltype_batch'].unique()
for ct in uniq_cts:
    file = "diff_peaks_" + ct.replace(" ", "_") + "_2vs18_iter.csv"
    if os.path.exists(file):
        print(f"Skipping {ct}, output file already exists.\n")
    ...
    group1 = ct + ":2mo"
    group2 = ct + ":18mo"
    g1 = data.obs['celltype_batch_age'] == group1
    g2 = data.obs['celltype_batch_age'] == group2
    if g1.sum() < 25 or g2.sum() < 25: 
        print("not enough cells\n")
        next
    peaks_selected = peaks[ct.split(":")[0]].to_numpy()
    diff_peaks = snap.tl.diff_test(
        peak_mat,
        cell_group1=g1,
        cell_group2=g2,
...
if __name__ == "__main__":
    ...
    num_processes = 20
    with Pool(num_processes) as pool:
        pool.map(process_ct_batch, uniq_cts)
...
##celltype batch region

data.obs['celltype_batch_region'] = data.obs['celltype_final']+":"+data.obs['batch']+":"+data.obs['region']
data.obs['celltype_batch_region_age'] = data.obs['celltype_batch_region']+":"+data.obs['age']

uniq_cts = data.obs['celltype_batch_region'].unique()
for ct in uniq_cts:
    file = "diff_peaks_" + ct.replace(" ", "_") + "_2vs18_iter.csv"
    if os.path.exists(file):
        print(f"Skipping {ct}, output file already exists.\n")
    ...
    group1 = ct + ":2mo"
    group2 = ct + ":18mo"
    g1 = data.obs['celltype_batch_region_age'] == group1
    g2 = data.obs['celltype_batch_region_age'] == group2
    if g1.sum() < 25 or g2.sum() < 25: 
        print("not enough cells\n")
        continue
    peaks_selected = peaks[ct.split(":")[0]].to_numpy()
...
data=snap.read_dataset("h5ads_final/combined_save.h5ads")
...
snap.pl.umap(data, color="best_celltype_fixed2", interactive=True, out_file = "best_celltype_fixed2.html", width =700, height = 550)
...
#peak_mat = snap.pp.make_peak_matrix(data, use_rep=peaks['Peaks'], file = "pmat_celltype_final_rep_01.h5ad")
peak_mat = snap.read('h5ads_final/pmat_celltype_final_rep_01.h5ad')


# In[ ]:


data.obs['best_celltype_age'] = data.obs['best_celltype_fixed2']+":"+data.obs['age']
...
ct = "IOL"
file = "diff_peaks_" + ct.replace(" ", "_") + "_2vsold_iter.csv"
print(ct)
print(file)
group1 = ct + ":2mo"
group2 = ct + ":18mo"
g1 = data.obs['best_celltype_age'] == group1
g2 = data.obs['best_celltype_age'] == group2
print(g1.sum())
print(g2.sum())
diff_peaks = snap.tl.diff_test(
    peak_mat,
...
data.obs['best_celltype_age'] = data.obs['best_celltype_age'].apply(lambda x: 'IOL:old' if x == 'IOL:9mo' else x)
data.obs['best_celltype_age'] = data.obs['best_celltype_age'].apply(lambda x: 'IOL:old' if x == 'IOL:18mo' else x)


# In[ ]:


diff_peaks_df.head(65)
...
data.obs['class'] = data.obs['celltype_final'].apply(lambda x: x.split(' ')[-1])
data.obs['class'] = data.obs['class'].apply(lambda x: 'Glut' if x == 'Glut-Sero' else x)
data.obs['class'] = data.obs['class'].apply(lambda x: 'Glut' if x == 'Dopa' else x)

data.obs['class'] = data.obs['class'].apply(lambda x: 'Gaba' if x == 'Neur' else x)
data.obs['class'] = data.obs['class'].apply(lambda x: 'Gaba' if x == 'Gaba-Chol' else x)


data.obs['class'] = data.obs['class'].apply(lambda x: 'NN' if x == 'Lymphoid' else x)

data.obs['class'].unique()
...
data.obs['class_batch_region'] = data.obs['class']+":"+data.obs['batch']+":"+data.obs['region']
data.obs['class_batch_region_age'] = data.obs['class_batch_region']+":"+data.obs['age']

uniq_cts = data.obs['class_batch_region'].unique()
for ct in uniq_cts:
    file = "diff_peaks_" + ct.replace(" ", "_") + "_2vs18_iter.csv"
    if os.path.exists(file):
        print(f"Skipping {ct}, output file already exists.\n")
    ...
    group1 = ct + ":2mo"
    group2 = ct + ":18mo"
    g1 = data.obs['class_batch_region_age'] == group1
    g2 = data.obs['class_batch_region_age'] == group2
    if g1.sum() < 25 or g2.sum() < 25: 
        print("not enough cells\n")
        continue
    diff_peaks = snap.tl.diff_test(
...
data.obs['celltype'] = data.obs['celltype_final'] 
data.obs['best_celltype_fixed2']=="IOL NN"
...
get_ipython().run_cell_magic('time', '', "marker_peaks = snap.tl.marker_regions(peak_mat, groupby='celltype_final', pvalue=.001)\n")


# In[ ]:


snap.pl.regions(peak_mat, groupby='celltype_final', peaks=marker_peaks, interactive=False,height=900, width = 900, out_file = "marker_genes_heat.pdf")


# In[ ]:


snap.pl.regions(peak_mat, groupby='celltype_final', peaks=marker_peaks, interactive=True,height=900, width = 1500, out_file = "marker_genes_heat_.001.html")
...
import csv
# Convert the dictionary to a list of rows for CSV
rows = zip(*([key] + value for key, value in marker_peaks.items()))

with open('marker_peaks.001.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    # Write the data
    for cell_type, genes in marker_peaks.items():
        for gene in genes:
...
ag = snap.tl.aggregate_X(peak_mat, groupby= 'celltype_final', normalize = "RPM", file = "celltype_RPM.h5ad")
