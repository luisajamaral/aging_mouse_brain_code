...
color_region  = color_region[c(2:3),]
...
logFC_summary$celltype = sapply(strsplit(as.character(logFC_summary$celltype), "--"), `[`, 1)
...
# Extract all cell types from file names
all_celltypes <- gsub(".csv", "", basename(deg_files))
...
for (file in deg_files) {
  ...
  # If the gene is missing, add it with default values
  if (nrow(gene_data) == 0) {
    gene_data <- data.frame(
      X = gene_of_interest,
      avg_log2FC = 0,  # Default to 0 logFC if missing
      p_val_adj = 1,   # Default to NS (non-significant)
      celltype = celltype
    )
  }
...
# Assign Clade categories
logFC_summary$clade <- "Glut"
logFC_summary$clade[grep("NN", logFC_summary$celltype)] <- "NN"
logFC_summary$clade[grep("Gaba", logFC_summary$celltype)] <- "Gaba"
logFC_summary$clade[grep("IMN", logFC_summary$celltype)] <- "NN"

# Extract Region and Cell Type from celltype
logFC_summary$region <- sapply(strsplit(as.character(logFC_summary$celltype), "--"), `[`, 2)
logFC_summary$celltype <- sapply(strsplit(as.character(logFC_summary$celltype), "--"), `[`, 1)

# Ensure all possible cell types are included, even if no data was found
all_combinations <- expand.grid(celltype = unique(logFC_summary$celltype),
                                region = unique(logFC_summary$region),
                                clade = unique(logFC_summary$clade))

# Merge to ensure all possible (celltype, region, clade) combinations exist
logFC_summary <- full_join(all_combinations, logFC_summary, by = c("celltype", "region", "clade"))

# Fill missing values with defaults (logFC = 0, NS)
logFC_summary <- logFC_summary %>%
  mutate(
    avg_log2FC = replace_na(avg_log2FC, 0),
    significance = replace_na(significance, "NS"),
...
# Plot logFC as a bar plot with facets
ggplot(logFC_summary, aes(x = celltype, y = avg_log2FC, fill = significance)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap( ~ region, scales = "free_x", ncol = 1) +  # <- Use wrap instead of grid
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +  
  scale_fill_manual(values = c("Downregulated" = "blue", "Upregulated" = "red", "NS" = "gray")) +
  theme_minimal() +
  labs(
    title = paste("LogFC of", gene_of_interest, "Across Cell Types"),
    x = "Cell Type",
...
#deg_results = fm
deg_results = read.csv("DEG_results_rm_rpl_rps/DG_Glut.csv")
deg_results$avg_log2FC = -deg_results$avg_log2FC 
deg_results$annotation = "Other"
deg_results$annotation[which(deg_results$X%in%lncRNA_genes)] = "lncRNA"
deg_results$annotation[which(deg_results$X%in%protein_coding_genes)] = "protein_coding"
...
df <- deg_results %>%
  mutate(
    significance = case_when(
      avg_log2FC > 0.25 & p_val_adj < 1e-5 ~ "Upregulated",
      avg_log2FC < -0.25 & p_val_adj < 1e-5 ~ "Downregulated",
       TRUE ~ "Not Significant"
    )
  )
table(df$significance)
...
options(repr.plot.width=6.5, repr.plot.height=5.5)
#deg_results = fm
deg_results = read.csv("DEG_results_rm_rpl_rps/STR_D12_Gaba.csv")
deg_results$avg_log2FC = -deg_results$avg_log2FC 
...
df <- deg_results %>%
  mutate(
    significance = case_when(
      avg_log2FC > 0.25 & p_val_adj < 1e-50 ~ "Upregulated",
      avg_log2FC < -0.25 & p_val_adj < 1e-50 ~ "Downregulated",
       TRUE ~ "Not Significant"
    )
  )
table(df$significance)
...
pdf("~/projects/combined_all/Figures/Figure4-Gaba//MA.pdf", height =5,width = 8)
ma_plot
dev.off()
...
print(combined_plot)
pdf("~/projects/combined_all/Figures/Figure5-DG/combined_MA.pdf", height =9,width = 6)
combined_plot
dev.off()
...
c_up = enrichGO(universe = df$X, gene = df$X[which(df$p_val_adj<1e-55 & df$avg_log2FC>0.25)],ont = "BP",keyType = "SYMBOL",
                       OrgDb = org.Mm.eg.db, 
                 pvalueCutoff = 0.1)
...
d1 = dotplot(sc_up,showCategory = 13, title = "Age-Up genes\nGO BP Enrichment")
d1
...
pdf( file = "~/projects/combined_all/Figures/Figure4-Gaba/up_GO_BP.pdf", height = 6.5, width = 6.5)
d1
dev.off()


#clust$Gene[which(clust$Cluster==1)]
c_down = enrichGO(universe = df$X, gene = df$X[which(df$p_val_adj<1e-55 & df$avg_log2FC< (-0.25))],ont = "BP",keyType = "SYMBOL",
                       OrgDb = org.Mm.eg.db, 
                 pvalueCutoff = 0.1)
...
d1 = dotplot(sc_down,showCategory = 10, title = "Age-Down genes\nGO BP Enrichment")
#pdf(d1, file = "~/projects/combined_all/Figures/Figure5-DG/down_GO_BP.pdf", height = 5, width = 6.5)
d1
#dev.off()

pdf(d1, file = "~/projects/combined_all/Figures/Figure4-Gaba/down_GO_BP.pdf", height = 6.5, width = 6.5)
d1
dev.off()
