...
setwd("~/ps-renlab2/projects/fmouse_multiome/SoloTE_out/")

all_directories <- list.dirs(path = ".", full.names = TRUE, recursive = TRUE)
data_directories <- all_directories[grep("class", all_directories)]
...
seuratobj = seuratobj[which(rs>=3),]
...
saveRDS(seuratobj, '~/ps-renlab2/projects/combined_all/female_RNA/SoloTE/HCP_SCT.RDS')

seuratobj = readRDS("~/ps-renlab2/projects/combined_all/female_RNA/SoloTE/HCA_SCT.RDS")
...
seuratobj$region = sapply(strsplit(as.character(seuratobj$sample), "_"), "[[", 1)

table(seuratobj$age )
table(seuratobj$region )
...
rownames(seuratobj)[grep("SoloTE",rownames(seuratobj))[1:5]]
...
setwd("~/ps-renlab2/projects/combined_all/female_RNA/SoloTE/")
...
seuratobj$celltype_sample = paste(seuratobj$celltype, seuratobj$sample, sep = "--")
...
goodct = names(table(seuratobj$celltype_sample)[which(table(seuratobj$celltype_sample)>300)])
head(goodct)
goodct = gsub("_", "-", goodct)
head(colnames(cm))
which(colnames(cm)%in% goodct)
cm = cm[,which(colnames(cm)%in% goodct)]


cmct = colnames(cm)
cmct = sapply(strsplit(as.character(cmct), "--"), `[`, 1)
...
pheatmap(cm[,which(clade=="Glut")], show_rownames = T,scale = "row",main = paste(nrow(cm), "TE"),annotation_col=annotation_col,
         color=colorRampPalette(c("navy", "white", "red"))(40) )



pheatmap(cm[,which(clade=="Gaba")], show_rownames = T,scale = "row",main = paste(nrow(cm), "TE"),annotation_col=annotation_col,
         color=colorRampPalette(c("navy", "white", "red"))(40) )



pheatmap(cm[,which(clade=="NN")], show_rownames = T,scale = "row",main = paste(nrow(cm), "TE"),annotation_col=annotation_col,
         color=colorRampPalette(c("navy", "white", "red"))(40) )
