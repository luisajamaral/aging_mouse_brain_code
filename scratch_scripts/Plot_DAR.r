...
setwd("~/projects/combined_all/female_RNA/DEG_results_latent_sample/")
logfc = 0.25
files = list.files(".", cl)
files = files[grep(".csv", files)]
all = list()
...
head(sampled_tab[which(sampled_tab$celltype=="Oligo_NN"),])
head(rsampled_tab[which(rsampled_tab$Celltype=="Oligo_NN"),])
