...
for i in unique(adata.obs['leiden-.25']) do: 

adata_c0 = adata[adata.obs['leiden-.25'] == '3'].copy()

snap.pp.select_features(adata_c0, n_features=250000)
snap.tl.spectral(adata_c0)
snap.pp.harmony(adata_c0, batch="batch", groupby='region', key_added='X_spectral_harmony')

snap.pp.knn(adata_c0,use_rep="X_spectral_harmony")
snap.tl.leiden(adata_c0)
snap.tl.umap(adata_c0,use_rep="X_spectral_harmony")

#snap.pl.umap(adata_c0, color="region", interactive=True, out_file = "Ast_subcluster_region.html")
snap.pl.umap(adata_c0, color="leiden", interactive=False, out_file = "Ast_subcluster_leiden.png")
snap.pl.umap(adata_c0, color="age", interactive=True, out_file = "Ast_subcluster_age.html")
snap.pl.umap(adata_c0, color="batch", interactive=False, out_file = "Ast_subcluster_batch.png")
snap.pl.umap(adata_c0, color="best_celltype", interactive=True, out_file = "Ast_subcluster_best_celltype.html")
snap.pl.umap(adata_c0, color="region", interactive=True, out_file = "Ast_subcluster_region.html")
snap.pl.umap(adata_c0, color="L2Annot.rough", interactive=True, out_file = "Ast_subcluster_L2Annot.rough.html")
snap.pl.umap(adata_c0, color="sample", interactive=True, out_file = "Ast_subcluster_sample.html")
snap.pl.umap(adata_c0, color="leiden-3", interactive=False,out_file = "Ast_subcluster_lieden-3.png")
...
df['L2Annot.rough'] = data.obs['L2Annot.rough']
df['age'] = data.obs['age']
df['subclass_label_v3'] = data.obs['subclass_label_v3']
df['best_celltype'] = data.obs['best_celltype']
df['region'] = data.obs['region']
df['batch'] = data.obs['batch']
df['leiden'] = data.obs['leiden']
df['leiden_1'] = data.obs['leiden_1']
df['leiden_2'] = data.obs['leiden-2']
df['leiden_3'] = data.obs['leiden-3']
...
df['umap_x'] = data.obsm['X_umap'][:,0]
df['umap_y'] = data.obsm['X_umap'][:,1]
df.to_csv("Ast_combined_meta.csv") 
data.close()
...
meta = pd.read_csv("meta_best_L2_allen.txt")


# In[ ]:


snap.pl.umap(data, color=meta['best_subclass_label_V3'], interactive=True, width =700, height = 550)
...
get_ipython().run_cell_magic('time', '', "snap.tl.macs3(data, groupby='leiden-2')\n")
...
get_ipython().run_cell_magic('time', '', 'peak_mat = snap.pp.make_peak_matrix(data, chunk_size=5000 , file = "peak_mat_iter.h5ad", peak_file= "filteredNfixed.union.peakSet.bed")\npeak_mat\n')
