...
data=snap.read_dataset("../h5ads_final/combined_save2_for_combinedDAR_rev.h5ads")
...
ages = np.array(obs_df['age'])

# Use numpy for vectorized operations
ages[np.where(ages == '9mo')] = 'old'
ages[np.where(ages == '18mo')] = 'old'
ages[np.where(ages == '2mo')] = 'yng'
...
celltype_final = np.array(obs_df['celltype_final'])
best_celltype_fixed2 = np.array(obs_df['best_celltype_fixed2'])
celltype_final[np.where(best_celltype_fixed2 == 'IOL')] = "IOL NN"
data.obs['celltype'] = celltype_final


# In[ ]:


data.obs['celltype_age'] = data.obs['celltype']+":"+data.obs['age']


# In[ ]:


data.obs['celltype_age'].unique()
...
def process_cell_type(ct):
    file = "diff_peaks_" + ct.replace(" ", "_") + "_2vsold.csv"
    background_file_csv = "background_peaks_" + ct.replace(" ", "_") + ".csv"
    background_file_bed = "background_peaks_" + ct.replace(" ", "_") + ".bed"
    ...
    background_peaks_df['pct_cells_yng'] = np.ravel(presence_g1)[background_indices] * 100
    background_peaks_df['pct_cells_old'] = np.ravel(presence_g2)[background_indices] * 100
    ...
    diff_peaks = snap.tl.diff_test(
        ...
        cell_group1=g2,
        cell_group2=g1,
        min_log_fc=0.1, min_pct=min_pct,
...
def process_existing_results(ct):
    file = "diff_peaks_" + ct.replace(" ", "_") + "_2vsold.csv"
    background_file_bed = "background_peaks_" + ct.replace(" ", "_") + ".bed"
    bed_up_file = "up_peaks_" + ct.replace(" ", "_") + "_0.1.bed"
    bed_down_file = "down_peaks_" + ct.replace(" ", "_") + "_0.1.bed"
    
    # Check if input files exist
    if not os.path.exists(file):
        print(f"Skipping {ct}, differential peaks file does not exist.\n")
        return

    diff_peaks_df = pd.read_csv(file)
    
    # Filter for significant peaks with adjusted p-value < 0.1
    significant_peaks = diff_peaks_df[diff_peaks_df['adjusted p-value'] < p_value_cutoff]
    
    # Separate up-regulated and down-regulated peaks
    up_peaks = significant_peaks[significant_peaks['log2(fold_change)'] > 0]
    down_peaks = significant_peaks[significant_peaks['log2(fold_change)'] < 0]
    
    if not up_peaks.empty:
        # Convert "chr:start-end" to BED format for up peaks
        up_peaks_bed = up_peaks['feature name'].str.split(r'[:-]', expand=True)
        up_peaks_bed.columns = ['chr', 'start', 'end']
        up_peaks_bed.to_csv(bed_up_file, sep="\t", header=False, index=False)
        ...
        print("Homer up")
        run_homer_analysis(bed_up_file, background_file_bed)
        print("Homer up annotation")
        run_homer_annotatepeaks(bed_up_file, background_file_bed)
    
    if not down_peaks.empty:
        # Convert "chr:start-end" to BED format for down peaks
        down_peaks_bed = down_peaks['feature name'].str.split(r'[:-]', expand=True)
        down_peaks_bed.columns = ['chr', 'start', 'end']
        down_peaks_bed.to_csv(bed_down_file, sep="\t", header=False, index=False)
...
def run_homer_annotatepeaks(bed_file, background_file):
    ...
    command = f"annotatePeaks.pl {bed_file} mm10 -go {output_dir}_go -genomeOntology {output_dir}_genOn -p 4 > {output_dir}.ann.txt"
    subprocess.run(command, shell=True, check=True)
    print(f"HOMER annotation completed for {bed_file}")

# Example cell types to process
for ct in ['IOL NN', 'OB-STR-CTX Inh IMN','DG-PIR Ex IMN' ]: 
    print(ct)
    process_existing_results(ct)
