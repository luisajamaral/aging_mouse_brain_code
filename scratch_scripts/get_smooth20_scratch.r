...
for (direction in c("Up", "Down")) {
  ...
  write.table(smthscore_table, row.names = FALSE, quote = FALSE, sep = "\t",
              file = paste("smooth/","smth20_scores_table_", direction, ".txt", sep = ""))
  }
  ...
  write.table(diff_cluster_table, row.names = FALSE, quote = FALSE, sep = "\t",
              file = paste("smooth/","clustered_diff_peaks_", direction, "_", pval, ".txt", sep = ""))
}


head(b)

head(diff_cluster_dict)
