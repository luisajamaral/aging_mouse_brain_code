...
data_directories = data_directories[grep("NAC", data_directories)]
...
merged_seuratobj <- CreateSeuratObject(count = solote_matrix,  min.cells = 5, min.features = 50, project = sample)
...
for (directory in data_directories[-1]) {
  ...
  temp_seuratobj <- CreateSeuratObject(count = solote_matrix, min.cells = 5, min.features = 50,project = sample)
  
  # Merge the current Seurat object with the merged_seuratobj
  merged_seuratobj <- merge(x=merged_seuratobj, y=temp_seuratobj,all = TRUE)
}
...
DimPlot(seuratobj,reduction="umap", group.by = "sample", label = T)

seuratobj = readRDS("~/projects/combined_all/female_RNA/SoloTE/HCP_SCT.RDS")
...
ps <- AggregateExpression(seuratobj, assays = "SCT", return.seurat = T, group.by = c("celltype", "sample"))
ps$age = sapply(strsplit(as.character(ps$sample), "-"), "[[", 2)
ps$celltype_age = paste(ps$celltype,ps$age, sep = "_")



counts = ps@assays$SCT$counts
...
cm = ae$SCT[which(rownames(ae$SCT)%in%all),]
cmct = colnames(cm)
cmct = gsub("-02mo", "", cmct)
cmct = gsub("-09mo", "", cmct)
cmct = gsub("-18mo", "", cmct)
